...
pub struct ConduitOpNode {
    ...
    pub args: RollupArgs,
    /// CLI historical RPC cutoff, taking precedence over genesis `conduit.migrationBlock`.
...
/// Uses upstream add-ons and historical forwarding with an independently configured cutoff.
...
impl<N> NodeAddOns<N> for ConduitOpAddOns<N>
where
    N: FullNodeComponents<Types: NodeTypes<ChainSpec = ConduitOpChainSpec>>,
    OpEthApiBuilder: EthApiBuilder<N>,
        ...
        NodeAddOns<N, Handle = <StandardOpAddOns<N> as NodeAddOns<N>>::Handle>,
{
    ...
    async fn launch_add_ons(mut self, ctx: AddOnsContext<'_, N>) -> eyre::Result<Self::Handle> {
        // Genesis alone does not require or enable an endpoint; an explicit CLI cutoff does.
        let cutoff = self.historical_rpc_block.or(ctx
            .config
            .chain
            .migration_block
            .filter(|_| self.inner.historical_rpc.is_some()));
        if let Some(block) = cutoff {
            // Remove the URL so upstream does not also install its Bedrock-derived layer.
